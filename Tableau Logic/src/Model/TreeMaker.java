...
public class TreeMaker {
	...
	/*
	 * Function which returns the root of the tree corresponding to the input expression
	 * if its syntax is correct
	 * form => The input expression to be analyzed
	 */
	public Formula syntaxAnalysis(String form) throws Exception{
		...
		for (String s : splitForm){
			...
			if (parenthese > 0) {
				...
				//If the all the parentheses have been closed we analyze the expression
				if (parenthese == 0) {
					...
					//Else it is a negation (the only one formula operator developed)
				...
				//We just create it and push it into the stack
					...
					switch(s){
						case "&":
							stack.push(new And(null, null, true));
							break;
						case "v":
							stack.push(new Or(null, null, true));
							break;
						case ">":
							stack.push(new Imply(null, null, true));
							break;
						case "<>":
							stack.push(new DoubleImply(null, null, true));
							break;
						//The last case is the Negation operator
						default:
							if (!stack.isEmpty()){
								if (stack.peek() instanceof Literal) {
							...
							stack.push(new Negation(null, true));
							break;
					}
				}
				else{
					throw new Exception("Invalid syntax here !");
				}
			}
		}
